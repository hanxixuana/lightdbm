...
/*!
* =======================================
* Xixuan: Poisson Loss for Delta Boosting
* =======================================
*/
/*!
* \brief Objective function for Poisson regression in Delta Boosting
*/
class RegressionPoissonDeltaLoss: public RegressionL2loss {
public:
    explicit RegressionPoissonDeltaLoss(const ObjectiveConfig& config): RegressionL2loss(config) {
      max_delta_step_ = static_cast<double>(config.poisson_max_delta_step);
      if (sqrt_) {
        Log::Warning("cannot use sqrt transform in %s Regression, will auto disable it.", GetName());
        sqrt_ = false;
    ...
    explicit RegressionPoissonDeltaLoss(const std::vector<std::string>& strs): RegressionL2loss(strs) {

    }

    ~RegressionPoissonDeltaLoss() {}
     ...
     *  hess = exp(f)
     *
     * And the output is exp(f); so the associated metrics get s=exp(f)
     * so that its loss = s - label * log(s); a little awkward maybe.
     *
     */
    void GetGradients(const double* score, score_t* gradients,
