...
class DeltaBoostingBernoulliLoss: public ObjectiveFunction {
    public:
        explicit DeltaBoostingBernoulliLoss(const ObjectiveConfig& config, std::function<bool(label_t)> is_pos = nullptr) {
          sigmoid_ = static_cast<double>(config.sigmoid);
          if (sigmoid_ <= 0.0) {
            Log::Fatal("Sigmoid parameter %f should be greater than zero", sigmoid_);
          }
        ...
        explicit DeltaBoostingBernoulliLoss(const std::vector<std::string>& strs) {
          sigmoid_ = -1;
          for (auto str : strs) {
            auto tokens = Common::Split(str.c_str(), ':');
            if (tokens.size() == 2) {
...
#pragma omp parallel for schedule(static)
        ...
        const char* GetName() const override {
          return "DeltaBoostingBernoulliLoss";
        }
